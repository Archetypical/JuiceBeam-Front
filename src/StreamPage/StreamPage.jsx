...
import { Confetti } from "../event_anim";
          ...
          <div className="ticker-outer">
            <div className="ticker-wrap">
              ...
              <div className="ticker">
                <span>Welcome to Juice Beam!</span>
              </div>
            </div>
          </div>
        </div>
        <div className="chat-column slideInRight normal">
          ...
          <div className={twitchChatOn ? "twitch-chat" : "twitch-chat d-none"}>
            <iframe
              frameBorder="0"
              id="twitch-chat-embed"
              src="https://www.twitch.tv/embed/jerma985/chat?parent=localhost"
            ></iframe>
          </div>
          {<BeamChat isOpen={beamChatOn} loggedIn={isLoggedIn} />}
        </div>
      </div>
...
function BeamChat(settings) {
  ...
  useEffect(() => {
    if (!settings.isOpen && document.getElementById("line0").innerHTML != "") beamChatClear();
    else if (settings.isOpen) init();
  });
  ...
  function beamChatStart() {
    ...
    dataText = [
      ...
      "-Enter 'juice' to see your Juice Stats.",
  ...
  function sanityCheck(text) {
    if (document.getElementById("beamChat").classList.contains("d-none")) {
      settings.isOpen = false;
      isTyping = false;
      beamChatClear();
    }
    if (text != dataText[lineNumber]) {
      isTyping = false;
      beamChatClear();
      if (currentMenu == "Main") beamChatStart();
      if (currentMenu == "Events") eventsInit();
      return true;
    }
    return false;
  }
  ...
  function typeWriter(text, i, fnCallback) {
    ...
    if (i < text.length) {
      ...
      document.querySelector("#line" + lineNumber).innerHTML = document.querySelector("#line" + lineNumber).innerHTML.replace('<span class=\"caret\" aria-hidden=\"true\"></span>', "");
      // add next character to h1
      document.querySelector("#line" + lineNumber).innerHTML = document.querySelector("#line" + lineNumber).innerHTML +
        nextChar +
  ...
  function colorText(line) {
    ...
    var lineText = document.querySelector("#line" + line).innerHTML;
    lineText = lineText.replace('<span class=\"caret\" aria-hidden=\"true\"></span>', "");
    var textArray = lineText.split(" ");
    textArray = textArray.filter(item => item);
      ...
      if (textArray[textArray.length - 1].includes("Confetti")) {
        lineText = lineText.replace(
          ...
          '<span style="color: #5fdaff;">' + textArray[textArray.length - 1] + "</span>"
        );
      ...
      if (textArray[textArray.length - 1].includes("Fire")) {
        lineText = lineText.replace(
          textArray[textArray.length - 1],
          '<span style="color: #ff9900;">' + textArray[textArray.length - 1] + "</span>"
        );
      ...
      if (textArray[textArray.length - 1].includes("'")) {
        lineText = lineText.replace(
          textArray[textArray.length - 1],
          '<span style="color:white;">' + textArray[textArray.length - 1] + "</span>"
        );
